...
BOOL CAT_Deinit(DWORD hDeviceContext)
{
	return TRUE;
} 
DWORD CAT_Init(DWORD dwContext)
{
	RETAILMSG(1,(TEXT("[	CAT9555++]\r\n")));
	Virtual_Alloc();
	PortConfig(0x40,CONFIG_PORT_0,0xff);
	PortConfig(0x40,CONFIG_PORT_1,0xff);
	RETAILMSG(1,(TEXT("[	CAT9555--]\r\n")));
	return TRUE;
}
#if 0
...
#endif
//-----------------------------------------------------------------------------
BOOL CAT_IOControl(DWORD hOpenContext, 
				   DWORD dwCode, 
				   PBYTE pBufIn, 
				   DWORD dwLenIn, 
				   PBYTE pBufOut, 
	...
	case IOCTL_CAT9555_WRITE:
		...
		if(*(UINT8 *)pBufIn<=7 && *(UINT8 *)pBufIn>=0)
		{
			if(port_0_status&(1<<*(UINT8 *)pBufIn))
			{
				...
				PortConfig(0x40,CONFIG_PORT_0,port_0_status);
			...
			{//output 1
				if(!(write_port_0_status&(1<<*(UINT8 *)pBufIn)))
				{
					...
					PortWrite(0x40,OUTPUT_PORT_0,write_port_0_status);
		...
		else
		{
			if(port_1_status&(1<<(*(UINT8 *)pBufIn-8)))
			{
				...
				PortConfig(0x40,CONFIG_PORT_1,port_1_status);
				//RETAILMSG(1,(TEXT("Config Port %x Output\r\n"),*(BYTE *)pBufIn));
			}
			if(*(BYTE *)pBufOut==0x1){//output 1
				if(!(write_port_1_status&(1<<(*(UINT8 *)pBufIn-8))))
				{
					...
					PortWrite(0x40,OUTPUT_PORT_1,write_port_1_status);
					//RETAILMSG(1,(TEXT("Write Port %x Level H\r\n"),*(BYTE *)pBufIn));
				}
			}else
			{
	...
	case IOCTL_CAT9555_READ:
		if(pBufOut && dwLenOut>=sizeof(BYTE))
		{
			...
			if(*(UINT8 *)pBufIn<=7 && *(UINT8 *)pBufIn>=0)
			{
				...
				if(!(PortRead(0x40,INPUT_PORT_0)&(1<<*(UINT8 *)pBufIn)))
					*(BYTE *)pBufOut=0x0;
				else
					*(BYTE *)pBufOut=0x1;				
			}
			else
			{
				...
				if(!(PortRead(0x40,INPUT_PORT_1)&(1<<(*(UINT8 *)pBufIn-8))))
					*(BYTE *)pBufOut=0x0;
				else
					*(BYTE *)pBufOut=0x1;
			}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD CAT_Open(DWORD hDeviceContext, DWORD AccessCode, DWORD ShareMode)
{
	//RETAILMSG(1,(TEXT("USERMUL: CAT_Open\r\n")));
	return TRUE;
} 

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
BOOL CAT_Close(DWORD hOpenContext)
{
	return TRUE;
} 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAT_PowerDown(DWORD hDeviceContext)
{
	//RETAILMSG(1,(TEXT("USERMUL: CAT_PowerDown\r\n")));
	
} 

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void CAT_PowerUp(DWORD hDeviceContext)
{
	//RETAILMSG(1,(TEXT("USERMUL: CAT_PowerUp\r\n")));

} 

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD CAT_Read(DWORD hOpenContext, LPVOID pBuffer, DWORD Count)
{
	//RETAILMSG(1,(TEXT("USERMUL: CAT_Read\r\n")));
	return TRUE;
} 

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD CAT_Seek(DWORD hOpenContext, long Amount, DWORD Type)
{
	//RETAILMSG(1,(TEXT("USERMUL: CAT_Seek\r\n")));
	return 0;
} 

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD CAT_Write(DWORD hOpenContext, LPCVOID pSourceBytes, DWORD NumberOfBytes)
{
	//RETAILMSG(1,(TEXT("USERMUL: CAT_Write\r\n")));
	return 0;
}
