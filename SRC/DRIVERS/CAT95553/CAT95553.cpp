...
BOOL ACT_Deinit(DWORD hDeviceContext)
{
	return TRUE;
} 
DWORD ACT_Init(DWORD dwContext)//----------------------------------------
{
	RETAILMSG(1,(TEXT("[	CAT95553++]\r\n")));
	Virtual_Alloc();
	PortConfig(0x44,CONFIG_PORT_0,0xff);
	PortConfig(0x44,CONFIG_PORT_1,0xff);
	RETAILMSG(1,(TEXT("[	CAT95553--]\r\n")));
	return TRUE;
}
#if 0
...
#endif
//-----------------------------------------------------------------------------
BOOL ACT_IOControl(DWORD hOpenContext, 
				   DWORD dwCode, 
				   PBYTE pBufIn, 
				   DWORD dwLenIn, 
				   PBYTE pBufOut, 
...
{	
	...
	switch(dwCode)
	{
	case IOCTL_CAT95553_WRITE://------------------------
		//RETAILMSG(1,(TEXT("[	CAT95553-- IOCTL_CAT95553_WRITE  ]\r\n")));
		...
		if(*(UINT8 *)pBufIn<=7 && *(UINT8 *)pBufIn>=0)
		{
			if(port_0_status&(1<<*(UINT8 *)pBufIn))
			{
				...
				PortConfig(0x44,CONFIG_PORT_0,port_0_status);//--------------------------------------------
			...
			{//output 1
				if(!(write_port_0_status&(1<<*(UINT8 *)pBufIn)))
				{
					...
					if(PortWrite(0x44,OUTPUT_PORT_0,write_port_0_status))
					{
					 RETAILMSG(1,(TEXT("[	CAT95553-- OK ]\r\n")));
					 }//---------------------------------------
					else{RETAILMSG(1,(TEXT("[	CAT95553-- FALSE]\r\n")));}
			...
			else
			{
				...
				{
					write_port_0_status &= ~(1<<*(UINT8 *)pBufIn);
					PortWrite(0x44,OUTPUT_PORT_0,write_port_0_status);//-------------------------------------
		...
		else
		{
			if(port_1_status&(1<<(*(UINT8 *)pBufIn-8)))
			{
				...
				PortConfig(0x44,CONFIG_PORT_1,port_1_status);//----------------------------
				//RETAILMSG(1,(TEXT("Config Port %x Output\r\n"),*(BYTE *)pBufIn));
			}
			if(*(BYTE *)pBufOut==0x1){//output 1
				if(!(write_port_1_status&(1<<(*(UINT8 *)pBufIn-8))))
				{
					...
					PortWrite(0x44,OUTPUT_PORT_1,write_port_1_status);//-----------------------------------
					//RETAILMSG(1,(TEXT("Write Port %x Level H\r\n"),*(BYTE *)pBufIn));
				}
			}else
			{
		...
		break;
	case IOCTL_CAT95553_READ://---------------------------
			//RETAILMSG(1,(TEXT("[	CAT95553-- IOCTL_CAT95553_READ  ]\r\n")));
		if(pBufOut && dwLenOut>=sizeof(BYTE))
		{
			...
			if(*(UINT8 *)pBufIn<=7 && *(UINT8 *)pBufIn>=0)
			{
				...
				if(!(PortRead(0x44,INPUT_PORT_0)&(1<<*(UINT8 *)pBufIn)))//-------------------------------
					*(BYTE *)pBufOut=0x0;
				else
					*(BYTE *)pBufOut=0x1;				
			}
			else
			{
				...
				if(!(PortRead(0x44,INPUT_PORT_1)&(1<<(*(UINT8 *)pBufIn-8))))//-----------------------
					*(BYTE *)pBufOut=0x0;
				else
					*(BYTE *)pBufOut=0x1;
			}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD ACT_Open(DWORD hDeviceContext, DWORD AccessCode, DWORD ShareMode)
{
	//RETAILMSG(1,(TEXT("USERMUL: CTA_Open\r\n")));
	//RETAILMSG(1,(TEXT("[	CAT95553-- OPEN  ]\r\n")));
	return TRUE;
} 

//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
BOOL ACT_Close(DWORD hOpenContext)
{
	return TRUE;
} 
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void ACT_PowerDown(DWORD hDeviceContext)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
void ACT_PowerUp(DWORD hDeviceContext)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD ACT_Read(DWORD hOpenContext, LPVOID pBuffer, DWORD Count)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD ACT_Seek(DWORD hOpenContext, long Amount, DWORD Type)
{
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DWORD ACT_Write(DWORD hOpenContext, LPCVOID pSourceBytes, DWORD NumberOfBytes)
{
