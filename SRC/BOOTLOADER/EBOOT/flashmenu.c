...
VOID BLWriteFlashLOGO(OAL_BLMENU_ITEM *pMenu)
{
	OAL_KITL_TYPE bkType;
	...
	bkType = g_eboot.type;
	g_eboot.bootDeviceType = BOOT_SDCARD_TYPE;
	if (FALSE == OEMReadData(1024*768*3+54, (UCHAR *) (IMAGE_WINCE_CODE_CA)))
	{
		OALLog(TEXT("SD boot failed to read file\r\n"));
		return;
	}
	...
	if (OEMWriteFlash(IMAGE_WINCE_CODE_CA, 1024*768*3+54))
		OALLog(L"BLWriteFlashXLDR success..\n");
	else
		OALLog(L"BLWriteFlashXLDR fail..\n");
...
OAL_BLMENU_ITEM g_menuFlash[] = {
    {
        ...
        L'd', L"Write LOGO", BLWriteFlashLOGO,
        NULL, NULL, NULL
    }, {
