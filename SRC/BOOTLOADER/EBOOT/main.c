    ...
    )
{
    ...
    ch7033=I2COpen(OMAP_DEVICE_I2C3);  //open i2c3
    I2CSetSlaveAddress(ch7033,0x76);       //set slaveaddress
	...
	I2CWrite(ch7033,3,&address_ch7033,1);
	I2CRead(ch7033, 0x50, &ch7033_id, 1);
	
	OALLog(L"the ch7033 id is %x\r\n",ch7033_id);
...
IsDVIMode()
{
    ...
    if (dispRes==OMAP_LCD_DEFAULT || dispRes==OMAP_DVI_800W_480H)
        return FALSE;
    else
        return TRUE;        
}
...
IsLVDSMode()
{
    DWORD dispRes;    
    GetDisplayResolutionFromBootArgs(&dispRes);
    if (dispRes==OMAP_DVI_800W_600H || dispRes==OMAP_DVI_800W_600H_56HZ)
        return TRUE;
    else
        return FALSE;        
}
