...
const NAND_INFO* BSPGetNandInfo(DWORD manufacturer,DWORD device)
{
	...
	if(manufacturer == 0xAD)
		return &SupportedNands[2];
...
// Functions to get bsp_args parameter 
DWORD BSPGetNANDECCType(VOID)
{
    BSP_ARGS *pArgs;
    PHYSICAL_ADDRESS pa;
    DWORD ECCtype=0;

    /* Get ECC mode settings */	
    pa.QuadPart = IMAGE_SHARE_ARGS_CA;
    pArgs	= (BSP_ARGS*)MmMapIoSpace(pa, sizeof(pArgs), FALSE);
    if (pArgs == NULL)
    {
        DEBUGMSG(ZONE_ERROR, (L"ERROR: "
            L"Failed map ARG parameters \r\n"));
        return FALSE;
    }
	
    ECCtype = pArgs->ECCtype; 
    if (pArgs != NULL)
    {
        MmUnmapIoSpace((void*)pArgs, sizeof(pArgs));
    }
    return ECCtype;
}
